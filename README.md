...
**Signing is a hook, not a step.** PRD Q3 — whether Quest will sign a tool carrying
unsupported status — is deferred. Run without the signing arguments and the script prints a
warning; the operator README leads with it. SEC9 requires a signature before distribution, so
an unsigned bundle is fine for a lab and is not releasable.
