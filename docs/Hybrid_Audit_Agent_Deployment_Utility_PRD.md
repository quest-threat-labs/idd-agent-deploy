...
| Q4 | Is a large single-file publish acceptable, or is a folder deployment preferred? | PM | **ANSWERED 2026-08-25 by measurement.** Folder. `PublishSingleFile` does not produce a single file for this project: the build leaves a 166 MB executable *plus* eight native libraries (`e_sqlite3.dll`, `pwrshplugin.dll`, `sni.dll` and others) and a `runtimes` directory that cannot be embedded. The operator copies a folder either way, so single-file buys nothing and adds a first-launch extraction step. Folder is 168 MB / 637 files before trimming and 143 MB / 314 files after; single-file is 162 MB but still 12 top-level entries. PRD §5.1 predicted this. |
